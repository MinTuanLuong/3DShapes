...
def show():
    ...
    geogebra_url1 = "https://www.geogebra.org/m/fz5ccvmb"
    if st.button("Tìm hiểu phần chung của mặt phẳng và hình cầu trên Geogebra"):
        st.write("Bấm vào nút bên dưới để mở trong tab mới.")
        st.markdown(f'<a href="{geogebra_url1}" target="_blank"><button>Geogebra - Hình cầu</button></a>', unsafe_allow_html=True)

    st.header("Công thức")
    st.write("Hãy cùng tìm hiểu công thức tính diện tích xung quanh và thể tích của hình cầu qua hoạt động dưới đây.")

    geogebra_url2 = "https://www.geogebra.org/m/ctbkghje"
    if st.button("Tìm hiểu các công thức của Hình cầu trên Geogebra"):
        st.write("Bấm vào nút bên dưới để mở trong tab mới.")
        st.markdown(f'<a href="{geogebra_url2}" target="_blank"><button>Geogebra - Hình cầu</button></a>', unsafe_allow_html=True)
